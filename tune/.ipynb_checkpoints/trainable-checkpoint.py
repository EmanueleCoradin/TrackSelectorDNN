...
from .utils_logging import create_run_dir, save_config, save_model_summary, CSVLogger, save_checkpoint
...
def trainable(config, checkpoint_dir=None):
    ...
    run_dir = create_run_dir(trial_name=trial_name)
    save_config(config, run_dir)
    
    # --- Load dataset ---
    dataset, collate = get_dataset(config)
    ...
    for epoch in range(n_epochs):
        ...
        # Ray Tune reporting
        if "tune" in globals() and tune.is_session_enabled():
            tune.report(**metrics)
